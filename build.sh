...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.USRP_UHD.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.USRP_UHD-4.0.0
        tar czf ${tmpdir}/rh.USRP_UHD-4.0.0.tar.gz --exclude=".svn" -C ${tmpdir} rh.USRP_UHD-4.0.0
        rpmbuild -ta ${tmpdir}/rh.USRP_UHD-4.0.0.tar.gz
        rm -rf $tmpdir
    else
