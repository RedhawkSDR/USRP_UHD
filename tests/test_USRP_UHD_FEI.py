...
# Uncomment these 3 lines if using IDE's "Run As --> Python unit-test" option
#DEVICE_INFO['SPD'] = '../USRP_UHD.spd.xml' # if script is placed in testing dir, auto-populated with correct spd 
#ossie.utils.testing.setImplId('cpp')
#ossie.utils.testing.setSoftPkg('../USRP_UHD.spd.xml')
...
class DeviceTests(ossie.utils.testing.ScaComponentTestCase):
    ''' FrontEnd device compatibility tests
        ...
        Customize getToBasicState function if your device has special start up requirements
        Customize getToShutdownState function if your device has special shut down requirements
    '''
    
    dut = None
    dut_ref = None
    device_discovery = {}
    device_discovery['TX'] = 0
    device_discovery['RX'] = 0
    device_discovery['CHANNELIZER'] = 0
    device_discovery['DDC'] = 0
    device_discovery['RX_DIGITIZER'] = 0
    device_discovery['RX_DIGITIZER_CHANNELIZER'] = 0
    device_discovery['UNKNOWN'] = 0
    ...
    def getToBasicState(self, execparams={}, configure={}, initialize=True):
        ''' Function used to launch device before each test case
            ...
            Add special start-up commands for your device to this function
        '''
        if not execparams:
        ...
        ### TODO - add device-specific startup commands here
    ...
    def getToShutdownState(self):
        ''' Function used to release device after each test case
            Add special shut-down commands for your device to this function
        '''
        ### TODO - add device specific shutdown commands here
    ...
    def FRONTEND_6(self):
        self.testReport.append('\nFRONTEND Test 6 - TX')
        self.testReport.append('Not implemented!')
    ...
    def FRONTEND_1_1_ExternalsTest(self):
        ...
        try:
            ...
            reason = 'analog'
        try:
            AnalogTuner = self.dut.getPort('AnalogTuner_in') # TODO AnalogTuner_in instead?
        except:
            AnalogTuner = None
            reason = 'digital'
    ...
    #Helpers
    def check(self, A, B, message, throwOnFailure=False, silentSuccess=False, indent_width=0, failureMsg='FAILURE', successMsg=u'\u2714'):
    ...
    def checkAlmostEqual(self, A, B, message, throwOnFailure=False, silentSuccess=False, indent_width=0, failureMsg='FAILURE', successMsg=u'\u2714', places=7):
        # successMsg suggestions: PASS, YES, u'\u2714' (check mark)
        # failureMsg suggestions: FAIL, NO, u'\u2718' (x mark)
    ...
    def _checkBasicAllocation(self,tuner_type):
        ...
        self.check(self.dut_ref.allocateCapacity(t1Alloc), True, 'Can allocate single %s'%tuner_type, True)
        
        if ddc_gen:
            # Allocate a single DDC on that channel
            ddc = ddc_gen(t1)
            ddcAlloc = self._generateAlloc(ddc)
            self.check(self.dut_ref.allocateCapacity(ddcAlloc), True, 'Can allocate single DDC', True)
        ...
        for t in range(0,self.device_discovery[tuner_type]):
            ...
            self.check(self.dut_ref.allocateCapacity(tAlloc), True, 'Allocating %s number: %s'%(tuner_type,t), True, True)
        self.check(True, True, 'Allocated to max %ss'%tuner_type)
        
        if ddc_gen:
            # Allocate to max DDCs for each tuner
            # TODO - add logic to handle DDCs that aren't mapped to each CHAN (i.e. apogee)
            ddcs = []
            for num,t in enumerate(ts):
                for d in range(0,DEVICE_INFO[tuner_type]['MAX_DDC_PER_CHAN']):
                    ddcs.append(ddc_gen(t))
                    ddcAlloc = self._generateAlloc(ddcs[-1])
                    self.check(self.dut_ref.allocateCapacity(ddcAlloc), True, 'Allocating DDC number: %s.%s'%(num,d), True, True)
    ...
    def _checkTunerPortIDL(self,ttype,tuner_info,controller,listener=None,analog_tuner=None,digital_tuner=None):
        ...
        self.check(tuner_control != None, True, 'Can get %s port'%(port_name), True)
        self.check(CORBA.is_nil(tuner_control), False, 'Port reference is not nil', True)
    ...
    def _checkAnalogDataFlow(self,ttype,controller,listener=None):
        ...
        if self.device_discovery[ttype] < 2:
            self.check(True,True,'RFInfo: Cannot fully test multiport because only single %s tuner capability'%(ttype))
        else:
    ...
    def _checkDigitalDataFlow(self,ttype,controller,listener1=None,listener2=None):
        ...
        for port in self.scd.get_componentfeatures().get_ports().get_uses():
            ...
            sri1 = dataSink1.sri()
            #print 'sri1',sri1
            self.checkAlmostEqual(status['FRONTEND::tuner_status::sample_rate'], 1.0/sri1.xdelta, '%s: SRI has correct value for xdelta'%(comp_port_name),places=0)
            self.check(status['FRONTEND::tuner_status::complex'],sri1.mode,'%s: SRI has correct value for mode'%(comp_port_name))
            ...
            if 'COL_RF' in keywords:
                ...
                if ttype == 'DDC':
                    if chan_status != None:
                        self.checkAlmostEqual(chan_status['FRONTEND::tuner_status::center_frequency'],keywords['COL_RF'],'%s: SRI has correct value for COL_RF keyword'%(comp_port_name),places=0)
                    else:
                        print 'WARNING - could not determine center frequency of collector to compare with COL_RF keyword'
                else:
                    self.checkAlmostEqual(status['FRONTEND::tuner_status::center_frequency'],keywords['COL_RF'],'%s: SRI has correct value for COL_RF keyword'%(comp_port_name),places=0)
            else:
            ...
            if 'CHAN_RF' in keywords:
                ...
                self.checkAlmostEqual(status['FRONTEND::tuner_status::center_frequency'],keywords['CHAN_RF'],'%s: SRI has correct value for CHAN_RF keyword'%(comp_port_name),places=0)
            else:
            ...
            if 'FRONTEND::BANDWIDTH' in keywords:
                ...
                self.checkAlmostEqual(status['FRONTEND::tuner_status::bandwidth'],keywords['FRONTEND::BANDWIDTH'],'%s: SRI has correct value for FRONTEND::BANDWIDTH keyword'%(comp_port_name),places=0)
            else:
            ...
            if 'FRONTEND::RF_FLOW_ID' in keywords:
                ...
                self.check(status['FRONTEND::tuner_status::rf_flow_id'],keywords['FRONTEND::RF_FLOW_ID'],'%s: SRI has correct value for FRONTEND::RF_FLOW_ID keyword'%(comp_port_name))
            else:
            ...
            if 'FRONTEND::DEVICE_ID' in keywords:
                ...
                #self.check(1,keywords['FRONTEND::DEVICE_ID'],'SRI has correct value for FRONTEND::DEVICE_ID keyword')
            else:
            ...
            if self.device_discovery[ttype] < 2:
                self.check(True,True,'%s: Cannot fully test multiport because only single %s tuner capability'%(comp_port_name,ttype))
            else:
                pass # TODO - additional multiport tests here
...
if __name__ == '__main__':
    ...
    #alternate way...
    #for fname in os.listdir(os.pardir):
    #    if fname.endswith('.spd.xml'):
    #        fname = os.path.abspath(os.path.join(os.pardir,fname))
    #        break
    #else:
    #    print 'ERROR - no SPD XML file found in parent directory. Executing in %s'%os.path.abspath(os.curdir)
    #ossie.utils.testing.main(fname)

    #yet another way... (hard-coded)
    #ossie.utils.testing.main('../USRP_UHD.spd.xml')
